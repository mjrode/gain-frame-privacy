...
description: "Interactive workflow for generating, structuring, and publishing SEO-optimized marketing blog posts for GainFrame. Use when the user says \"write a blog post\", \"new blog post\", \"blog-post-generator\"."
---
...
#### Screenshot Library Catalog (v1.21 — 14 screenshots)
...
| `muscle-map.png` | Muscle map (large transformation state) — Front/Back toggle, BEFORE (yellow/red highlighted) vs AFTER (mostly green) body silhouettes, gradient legend (Needs Work → Developing → Solid → Strong → Elite), radar chart of 7 muscle areas (Front Delts, Side/Rear Delts, Upper Chest, Mid/Lower Chest, **Biceps**, Abs, Obliques) | Per-muscle scoring, 12 muscle group ratings, the "Developing → Strong" progression labels, weak-point identification, big transformation visualization. **Use this when the article needs a "lots of progress" visual.** |
| `muscle-compare.png` | Muscle comparison (in-progress state) — same screen as `muscle-map.png` with a less-progressed user: BEFORE shows yellow/red, AFTER shows partial green only on shoulders/abs. Radar chart shows baseline (dotted) vs current (filled) with 7 muscle areas including **Triceps** (not Biceps). | Muscle comparison content where the focus is the *tracking mechanism* over the result — radar baseline-vs-current visualization, ongoing-progress content, "what muscle tracking looks like mid-journey." **Use this when the article is about the comparison feature itself, not a finished transformation.** |
...
| `day-checkin-score.png` | Day detail / check-in score card — full daily-view with photo at top (1:24 PM, 1/7 photos), "Unlock Deep Insights" upgrade CTA, score card ("IMPRESSIVE" badge, 68 score / 7d avg 68.5 / 18.0% BF / 239.0 lbs), narrative ("Strong shoulder width and chest fullness with tightening midsection"), Precision Body Fat status card, **Hevy workout integration card** showing the day's lifting (Chest, 31min · 4 exercises · 16,890lbs volume) with set-by-set breakdown | The daily check-in workflow, Hevy integration content, "what your check-in day looks like," Deep Insights paywall content, narrative AI feedback paired with workout data, the photo+score+workout loop |
...
| `post-check-in-photo-score.png` | Score card detail (isolated) — "IMPRESSIVE" trend badge, GainFrame Score 68 with body fat 17.0% + weight 239 lbs, narrative feedback ("Strong shoulder width and chest fullness with visible midsection leanness"), 4-metric breakdown (Body Fat 65 / Muscle 72 / Proportions 70 / Goal Fit 68), trajectory section ("Recomp On Track") | The Deep Dive Report, score breakdown, narrative AI feedback, the 4-metric framework, "what your score means" content. **Use this when you want the score card without surrounding day context** (use `day-checkin-score.png` if you want photo + score + Hevy together). |
| `macros.png` | Plan tab — Cut/Maintain/Bulk goal selector with calculated daily targets (Cut 2,722 / Maintain 3,222 / Bulk 3,722, base RMR 2,070), macro breakdown bars (Protein 211g / Carbs 393g / Fat 88g totaling 3,222 kcal), AI training recommendation card ("Lateral Delt Isolation" with set/rep prescription and rationale), "Your Priority" narrative card interpreting recomp progress | Macro tracking content, calorie targets, AI training recommendations / Target Training, recomp/cut/bulk planning, the "what to actually do next" content arc, narrative-AI-as-coach content |
| `ai-coach.png` | GainFrame Coach chat screen — "New conversation" header (General chat subtitle), "Coach Knows" summary chip showing current data (17.0% BF / 58 GF / 239.0 lb), "Ask anything about your check-in" empty-state headline ("Coach uses your real numbers. It won't make any up."), 3 suggested prompt cards ("Why did my score drop?", "How am I trending overall?", "What's my biggest area for improvement?"). Includes App Store framing with "Chat with your AI Coach" caption above the phone. | The GainFrame Coach feature specifically, AI fitness coach articles, AI chat assistants for fitness, "data-grounded AI vs ChatGPT" comparison content, the Pro-tier coaching experience |
...
6. **Update Index:** No manual update needed. The blog index cards are auto-generated by `web/scripts/generate-blog-grid.mjs` (registered as a `prebuild` hook in `web/package.json`). Every `next build` and `npm run deploy` runs it automatically — it reads frontmatter from every `.mdx` in `web/content/blog/`, sorts by `displayDate` descending, and replaces the cards between the `<!-- BLOG_GRID:START -->` ... `<!-- BLOG_GRID:END -->` markers in `web/lib/_extracted/blog-body.html`.

   **Frontmatter fields the script reads** (in fallback order):
   - **Card text:** `subtitle` → `description` → empty
   - **Cover alt:** `coverAlt` → `description` → `title`
   - **Category badge:** `displayCategory` → `breadcrumbCategory` → "Article"
   - **Date:** `displayDate` (string, parsed with `new Date()` for sort key)
   - **href:** `slug` → filename without `.mdx`

   To control the card text precisely, set `subtitle` in the frontmatter (most posts already do). To override cover alt with bespoke description, add a `coverAlt:` field — otherwise the alt falls back to `description` which is fine for accessibility.

   To regenerate the index manually without building, run `npm run build:blog-grid` from `web/`.
