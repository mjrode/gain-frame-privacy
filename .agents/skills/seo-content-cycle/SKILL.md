...
| `mcp__e6e8a8f4-…__*` (DataForSEO) | **Works when connected**, but can be absent from a session even when GA4 and PostHog are available (first seen 2026-08-11). Live, paid, per-call. If no DataForSEO tools are exposed, record the source as unavailable, do not reuse prior volume/KD as current-run data, and do not propose a post whose case depends on missing market or SERP evidence | Real search volume, keyword difficulty, search intent, 12-month trend, SERP composition, competitor keyword gaps, AI Overview presence, backlinks |
