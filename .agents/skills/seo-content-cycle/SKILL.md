...
|---|---|---|
| `mcp__gsc__*` | **Works when connected**, but the server (`pipx run mcp-gsc`, defined in `~/.claude.json`) is **absent from some sessions** (first seen 2026-08-07). `sc-domain:gainframe.app` shows as `siteFullUser` | All GSC performance, queries, pages, index verdicts, URL inspection. **This is the performance source for this site.** If the tools are missing (ToolSearch finds nothing for `select:mcp__gsc__*`), do NOT stop and do NOT touch the SEO Receipts connector — call the Search Console API directly: the service account key is `~/.config/gainframe/gsc-service-account.json`, scope `https://www.googleapis.com/auth/webmasters`. As of 2026-08-29, neither `/usr/bin/python3` nor the active pyenv Python has `google-auth`; use `/Users/michael.rode/.local/pipx/venvs/mcp-gsc/bin/python`, which carries the dependency with the installed server. POST `searchAnalytics/query` on `sites/sc-domain%3Againframe.app` for analytics and `v1/urlInspection/index:inspect` for verdicts — a ~40-line scratchpad helper covers the whole phase (see `seo-tools/content-audits/2026-08-07.md`) |
| `mcp__dataforseo__*` (official DataForSEO remote MCP) | **Registered in Codex as `dataforseo` at `https://mcp.dataforseo.com/mcp` and authenticated with OAuth.** Cursor sessions often have **no DataForSEO tools in the MCP catalog** (confirmed 2026-08-18). At the start of a run, `GetMcpTools` with pattern `dataforseo` (Cursor) or `codex mcp get dataforseo` (Codex). If missing in Cursor, do not stall and do not backfill: GSC still runs via the service-account fallback, and **every post whose case needs volume/KD/SERP is withheld**. If Codex reports `Auth: Not logged in`, run `codex mcp login dataforseo`; on 2026-08-23 the browser authorization completed automatically. The current task's MCP catalog does **not** hot-load tools after that login, so continue in a fresh task or a fresh read-only Codex subprocess before declaring the source unavailable. If it is registered in Codex but absent, also inspect `codex mcp list` for a disabled state. Only after these repair attempts fail should the run record the source as unavailable. Never reuse prior volume/KD as current-run data | Real search volume, keyword difficulty, search intent, 12-month trend, SERP composition, competitor keyword gaps, AI Overview presence, backlinks |
...
| `mcp__analytics-mcp__*` (GA4) | Available when the connector is up; this Cursor session used on-disk `analytics/raw/YYYY-MM-DD/` TSVs instead. **Stamp each file's window.** A 21-day GA4 export is not comparable to the GSC 28d/prior-28d pair (seen 2026-08-18). When the MCP tools are absent, the read-only GA4 Data API fallback works with `/Users/michael.rode/.config/gainframe/ga-mcp-key.json`, property `526084497`, scope `https://www.googleapis.com/auth/analytics.readonly`, and `/Users/michael.rode/.local/pipx/venvs/mcp-gsc/bin/python`; POST `v1beta/properties/526084497:runReport` and filter `sessionDefaultChannelGroup = Organic Search`. The raw summary key `ga4_recent_21d` can contain a 28-day window, so trust its stamped `start`, `end`, and `days`, not the legacy key name (confirmed 2026-08-29) | Organic sessions and engagement by landing page. Use as a separate behavioural layer in every decision run; never substitute it for GSC clicks |
...
| 2026-08-23 | DataForSEO was registered but OAuth had expired; login repaired it, while the current task catalog still could not hot-load the newly available tools | Data sources table — exact `Auth: Not logged in` symptom, `codex mcp login dataforseo`, and fresh-task/subprocess continuation |
