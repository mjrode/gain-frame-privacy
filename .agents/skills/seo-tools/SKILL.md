...
| 6 | **seo-content-cycle** | `/seo-content-cycle` (add `auto` to skip the approval gate) | Orchestrator that closes the loop: pulls GSC performance + index verdicts (`mcp__gsc__*`, property `sc-domain:gainframe.app`), layers **real DataForSEO** volume/difficulty/intent/trend/SERP data on top, runs the offline `content-inventory.mjs` for orphans + cannibalization + Quick Answer defects, then proposes numbered posts and fixes and **stops for approval**. On approval it delegates writing to #5a/#5b, verifies, ships, and submits to IndexNow. Every run leaves a dated audit + updates rolling `strategy.md`. | `content-audits/strategy.md`, `topical-map.md`, `TODO_SEO.md`, `product-context.md`, `writer-spec.md`, `mcp__gsc__*`, DataForSEO MCP, `content-inventory.mjs` | `content-audits/[date].md`, `content-audits/strategy.md`, updates `topical-map.md` + `TODO_SEO.md`; posts via #5a/#5b | Bi-weekly–monthly |
...
**Free signals, with one paid exception.** Skills #1–#5 require no paid API. They use Google autocomplete (free JSON endpoint), `WebSearch` for SERPs, `WebFetch` for sitemaps/homepages, and Google Search Console data (live via `mcp__gsc__*` tools when configured — property: `sc-domain:gainframe.app` — or CSV fallback at `gsc-data/queries.csv`). Trade-off: their volume numbers are *relative tiers* (High/Med/Low), not absolute monthly searches, and every report carries an honest-limitation callout to that effect.

`seo-content-cycle` (#6) is the exception: it calls the **DataForSEO** MCP for real search volume, keyword difficulty, search intent, 12-month trend, and SERP composition. Those calls cost money per request, so that skill batches keyword scoring (up to 700 keywords per call) and caps live-SERP lookups. **When #4 and #6 disagree on a keyword's value, #6 wins** — measured volume beats an estimated tier.

**Do not use the SEO Receipts connector (`mcp__2676e48c-…`) for this site.** It is OAuth-bound to `sc-domain:seoreceipts.com`, a different property. It returns clean-looking numbers for the wrong website.
...
| Standing "what should we publish next?" review | `/seo-content-cycle` — measures first, proposes with real volume/difficulty, waits for approval |
| Same, but you want it written and shipped in one pass | `/seo-content-cycle auto` |
...
│   ├── TODO_SEO.md                                 ← #4 keyword backlog (consumed by #5a, #5b, #6)
│   ├── topical-map.md                              ← cluster map + standing gap list (read by #4, #6)
│   ├── writer-spec.md                              ← canonical post invariants (read by #5a, #5b, #6)
│   ├── content-inventory.mjs                       ← offline link-graph / cannibalization / freshness checker (#6)
│   ├── gsc-data/queries.csv                        ← user-provided GSC export (fallback for #4)
│   ├── competitor-research/
...
│   └── content-audits/
│       ├── [date].md                               ← #6 per-run audit record
│       └── strategy.md                             ← #6 rolling state across runs (read first, written last)
├── docs/
│   ├── app-screenshots/1.21/                       ← screenshot library used by #5a
│   └── blog/[slug]/assets/                         ← images for #5a, #5b posts (symlinked into web/public/blog)
├── web/content/blog/[slug].mdx                     ← #5a, #5b output (blog index + sitemap auto-generated)
└── web/app/sitemap.ts                              ← generates sitemap from content/blog (no manual edit for posts;
                                                       new /tools/ pages DO need a hand edit to CALC_SLUGS)
```
