---
name: tiktok-apps
description: "Generates GainFrame app-listicle TikTok carousels (editorial \"Top 5\" and ranked countdown) where GainFrame is planted among popular fitness apps. Pulls real logos/screenshots/ratings from the iTunes API and composites deterministic slides with Pillow — no AI image generation. Use when the user says \"tiktok apps\", \"tiktok-apps\", \"app listicle\", \"apps carousel\", \"top 5 apps\", \"apps ranked\", \"fitness apps post\"."
---
...
| File | Purpose |
|------|---------|
| `.agents/skills/tiktok-apps/fetch_apps.py` | Resolves apps via iTunes API, downloads assets, writes `manifest.json` |
| `.agents/skills/tiktok-apps/render.py` | Composites all slides + `content.md` from the manifest (both formats) |
