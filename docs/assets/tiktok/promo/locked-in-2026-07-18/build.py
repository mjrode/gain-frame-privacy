#!/usr/bin/env python3
"""
Build the 2-slide "locked in" promo carousel.

Slide 1: real before photo (cropped from the deep-dive compare screenshot,
         date + BF% overlay left intact for credibility) — "I ALMOST GAVE UP"
Slide 2: real deep-dive payoff (photos + 48→78 score card) — "THEN I LOCKED IN"
...
SRC = os.path.join(HERE, "../../../tiktok-screenshots/deep-dive-compare.png")
...
def main():
    src = Image.open(SRC)
    before = src.crop((14, 182, 636, 916))          # heavy before photo, overlay intact
    # paint out the sheet drag-handle sliver caught in the top-right corner,
    # row by row so the fill follows the wall's shading gradient
    bd = ImageDraw.Draw(before)
    for y in range(0, 36):
        bd.line([548, y, 622, y], fill=before.getpixel((542, y)))
    payoff = src.crop((0, 170, 1290, 1560))          # both photos + 48→78 score card
