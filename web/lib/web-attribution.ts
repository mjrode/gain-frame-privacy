...
import { appStoreUrlWithCampaign } from "./site.ts";
...
/**
 * Build a privacy-safe AppsFlyer OneLink that preserves the website's first
 * page, download page, CTA, acquisition source, and anonymous PostHog IDs.
 * The deep_link_sub mapping is intentionally stable; the iOS app decodes the
 * same fields for first-touch storage, RevenueCat attributes, and Slack.
 */
export function buildWebAttributionLink(
  ...
  const currentUrl = runtime.currentUrl ??
    (typeof window !== "undefined" ? window.location.href : "https://gainframe.app/");
  const currentPath = (() => {
    try {
  ...
  const payload: WebAttributionPayload = {
    ...
    campaign: clean(context.utm_campaign ?? options.campaign, 120) ?? "owned_web",
  ...
  if (!oneLinkUrl) {
    return { href: appStoreUrlWithCampaign(options.campaign), payload };
  }

  const url = new URL(oneLinkUrl);
  // Keep the page/placement campaign on the rewritten anchor itself. The
  // delegated click tracker reads this on repeat clicks after the original
  // App Store URL has already been replaced by a OneLink URL.
...
export function isGainFrameDownloadUrl(href: string): boolean {
  try {
    ...
    return (
      (url.hostname === "apps.apple.com" && url.pathname.includes("6759252082")) ||
      (url.hostname === "gainframe.onelink.me" && url.pathname.startsWith("/WufP")) ||
