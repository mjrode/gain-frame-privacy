---
name: Google Search Console Report
description: Pull fresh GSC data for gainframe.app over the last N days — trend, top articles, top queries, movers — and cross-reference recent site changes. Read-only.
triggers:
  - "google-search-console"
  - "/google-search-console"
  - "gsc report"
  - "pull search console"
  - "how is search trending"
---
...
One-command organic search report for gainframe.app. Invoked as `/google-search-console [N]` where `N` = number of days to report on (default **7**). Uses the `gsc` MCP server (tools named `mcp__gsc__*` — load via ToolSearch if deferred). Read-only: never submits sitemaps, never deletes anything.
